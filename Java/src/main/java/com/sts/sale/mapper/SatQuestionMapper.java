...
public interface SatQuestionMapper extends BaseMapper<SatQuestion> {
     ...
     * @return 题目列表
     */
    @Select("SELECT * FROM sat_questions ORDER BY RAND() LIMIT #{limit}")
     ...
     * @return 题目列表
     */
    @Select("SELECT * FROM sat_questions WHERE domain = #{domain} ORDER BY RAND() LIMIT #{limit}")
     ...
     * @return 领域列表
     */
    @Select("SELECT DISTINCT domain FROM sat_questions WHERE domain IS NOT NULL")
